...
/*
 *  call-seq:
 *    gets(sep = $/, limit = nil, chomp: false) -> string or nil
 *    gets(limit, chomp: false) -> string or nil
 *
 *  Reads and returns a line, if possible, from the stream;
 ...
 *    f.close
 *
 *  With string argument +sep+ given,
...
/* Shared argument handling for the io_readline primitive: disambiguates the
 * sep/limit overload, applies encoding checks, and reads the line. Does not
 * touch $_ and does not raise on EOF; the caller does that. */
static VALUE
